...
class Bugs : AdventOfCodeTask {
    ...
    override fun run(part2: Boolean): Any {
        ...
        levels = if (part2) (-(rounds/2)..rounds/2).map{ it to emptyLevel.toMutableMap() }.toMap().toMutableMap() else mutableMapOf()
    ...
    private fun turn(recursive: Boolean) {
        ...
        levels.forEach { (level, map) ->
            ...
            map.forEach { (coordinate, c) ->
                ...
                val count = adjacent.count { (adj, lvl) -> levels.getOrDefault(lvl, emptyLevel.toMutableMap()).getOrDefault(adj, '.') == '#' }
                if (c == '#' && count != 1) {
                    updated[coordinate] = '.'
                }
            ...
            adjacent.flatMap {
                when {
                    ...
                    (it == center) -> when {
                        ...
                        (coordinate.y == center.up(offset = true).y) -> (0..maxX).map { x -> Coordinate(x, 0) to level + 1 }
                        (coordinate.y == center.down(offset = true).y) -> (0..maxX).map { x -> Coordinate(x, maxY) to level + 1 }
